...
get_date_time() {
	...
	date=$(echo "$filename" | grep -oE '.{10}-birdnet' | sed 's/_/-/g' | sed 's/-birdnet//')

	# Extract time (e.g., 06:32:54) from filename
	time=$(echo "$filename" | grep -oE '[0-9]{2}:[0-9]{2}:[0-9]{2}')
}
...
main_directory="$HOME/pCloud Drive/Miscellaneous/Apps/BirdNet-Pi/Detection_Database_Backups/BirdSongs/Extracted/By_Date"
...
IFS=$'\n'
# files=$(find "$main_directory" -type f -name "*birdnet*")
files=$(find "$main_directory" -type f -path "*2023-03-*" -o -path "*2023-06-*" -o -path "*2023-07-*" -o -path "*2023-04-01*" -o -path "*2023-04-02*" -o -path "*2023-04-03*" -o -path "*2023-04-04*" -o -path "*2023-04-05*" -o -path "*2023-04-06*" -name "*-birdnet-*")
